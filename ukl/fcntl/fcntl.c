...
extern long do_fcntl(int fd, unsigned int cmd, unsigned long arg, struct file *filp);
extern int check_fcntl_cmd(unsigned cmd);

static long sys_fcntl(int fd, unsigned int cmd, unsigned long arg)
{
    struct fd   f = fdget_raw(fd); /* Get the file associated with the file descriptor */
    long        err = -EBADF;
...
int fcntl(int fd, int cmd, ...)
{
    ...
    if (fd >= 0 && fd <= 2)
    {
        printk(KERN_ERR "Trying to call fcntl() with %d file descriptor\n", fd);
        return -EINVAL;
    }
